...
function vectorize(form) {
  let c = +form['complexity'].value;
  let ingrCount = countIngredients(form['ingredients-text'].value);
  let mealSelected = Array.from(document.querySelectorAll("input[name='meal']:checked")).map(x => x.value);
  let meal = mealSettings.map(ms => mealSelected.includes(ms)?1:0);
  let price = extractPrice(form['price'].value);
  let relSelected = Array.from(document.querySelectorAll("input[name='rel']:checked")).map(x => x.value);
  let rel = relationships.map(r => relSelected.includes(r)?1:0);
  let spice = spiceMap[form['spice'].value]||0;

  let nums = [c, ingrCount, ...meal, price, ...rel, spice];

  let text = cleanText(form['ingredients-text'].value)
           + ' ' + cleanText(form['movie'].value)
           + ' ' + cleanText(form['drink'].value);
...
document.getElementById('predict-form')
  .addEventListener('submit', e => {
    e.preventDefault();
    let vec = vectorize(e.target);
    let logits = labels.map((_,i) =>
      vec.reduce((sum, v, j) => sum + v * W[j][i], 0)
    );
    let probs = softmax(logits);
    let idx = probs.indexOf(Math.max(...probs));
    document.getElementById('result')
      .textContent = `Prediction: ${labels[idx]} (${(probs[idx]*100).toFixed(1)}%)`;
  });
